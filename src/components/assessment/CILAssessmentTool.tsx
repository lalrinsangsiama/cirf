        ...
        ;(async () => {
          try {
            ...
            if (response.ok && apiResult.data) {
              ...
              // Keep the draft so nothing is lost, and tell the user
              logger.error('Failed to save assessment', { error: apiResult.error?.message || apiResult.message })
              toastError(apiResult.error?.message || 'We couldn\'t save your results to your account. Your draft is preserved — please try again later.')
            }
          } catch (error) {
...
https://culturalinnovationlab.org/tools
    `.trim()
